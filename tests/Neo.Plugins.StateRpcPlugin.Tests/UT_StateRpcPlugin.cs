...
//
// UT_StateRpcPlugin.cs file belongs to the neo project and is free
...
namespace Neo.Plugins.StateRpcPlugin.Tests
{
    [TestClass]
    public class UT_StateRpcPlugin
    {
        ...
        private StateRpcPlugin? _stateRpcPlugin;
        private Neo.Plugins.StateRootPlugin.StateRootPlugin? _stateRootPlugin;
        ...
        public void Setup()
        {
            ...
            var onSystemLoaded = typeof(Neo.Plugins.StateRootPlugin.StateRootPlugin).GetMethod("OnSystemLoaded", bindingFlags);
            ...
            // Then load StateRpcPlugin
            _stateRpcPlugin = new StateRpcPlugin();
            var onSystemLoadedRpc = typeof(StateRpcPlugin).GetMethod("OnSystemLoaded", bindingFlags);
            Assert.IsNotNull(onSystemLoadedRpc, "OnSystemLoaded method not found via reflection.");
            onSystemLoadedRpc.Invoke(_stateRpcPlugin, [_system]);
            ...
            StateRpcSettings.Load(config);
            Assert.IsTrue(StateRpcSettings.Default.FullState);
        }

        [TestCleanup]
        public void Cleanup()
        {
            _stateRpcPlugin?.Dispose();
        ...
        {
            var result = _stateRpcPlugin!.GetStateHeight();
        ...
        {
            var exception = Assert.ThrowsExactly<RpcException>(() => _stateRpcPlugin!.GetStateRoot(999));
        ...
        public void TestGetProof_WithInvalidKey_ShouldThrowRpcException()
        {
            ...
            var exception = Assert.ThrowsExactly<RpcException>(() => _stateRpcPlugin!.GetProof(rootHash, scriptHash, invalidKey));
        ...
        public void TestVerifyProof_WithInvalidProof_ShouldThrowRpcException()
        {
            ...
            var exception = Assert.ThrowsExactly<RpcException>(() => _stateRpcPlugin!.VerifyProof(rootHash, invalidProof));
            Assert.AreEqual(RpcError.InvalidParams.Code, exception.HResult);
        }
        ...
        public void TestGetStateRoot_WithMockData_ShouldReturnStateRoot()
        {
            ...
            var result = _stateRpcPlugin!.GetStateRoot(1);
        ...
        public void TestGetProof_WithMockData_ShouldReturnProof()
        {
            ...
            var result = _stateRpcPlugin!.GetProof(rootHash, scriptHash, Convert.ToBase64String([0x01, 0x02]));
        ...
        public void TestGetState_WithMockData_ShouldReturnValue()
        {
            ...
            var result = _stateRpcPlugin!.GetState(rootHash, scriptHash, [0x01, 0x02]);
            Assert.IsNotNull(result);
            Assert.IsInstanceOfType(result, typeof(JString));
        ...
        public void TestFindStates_WithMockData_ShouldReturnResults()
        {
            ...
            var result = _stateRpcPlugin!.FindStates(rootHash, scriptHash, []);
            Assert.IsNotNull(result);
            Assert.IsInstanceOfType(result, typeof(JObject));
