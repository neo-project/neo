...
//
// UT_NeoBuildWalletInvalidPasswordException.cs file belongs to the neo project and is free
...
{
    [TestClass]
    public class UT_NeoBuildWalletInvalidPasswordException
    {
        [TestMethod]
        public void TestIsPropertiesSet()
        {
            var expectedMessageString = "Password is Invalid.";

            var exception = new NeoBuildWalletInvalidPasswordException();
            var exceptionInterface = exception as INeoBuildException;
            var exceptionBase = exception as Exception;

            // NeoBuildWalletInvalidPasswordException
            Assert.AreEqual(NeoBuildErrorCodes.Wallet.InvalidPassword, exception.ExitCode);
            ...
            // INeoBuildException
            Assert.AreEqual(NeoBuildErrorCodes.Wallet.InvalidPassword, exceptionInterface.ExitCode);
            ...
            // System.Exception
            Assert.AreEqual(NeoBuildErrorCodes.Wallet.InvalidPassword, exceptionBase.HResult);
