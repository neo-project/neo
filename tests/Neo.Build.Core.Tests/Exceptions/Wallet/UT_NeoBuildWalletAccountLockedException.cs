...
//
// UT_NeoBuildWalletAccountLockedException.cs file belongs to the neo project and is free
...
{
    [TestClass]
    public class UT_NeoBuildWalletAccountLockedException
    {
        [TestMethod]
        public void TestIsPropertiesSet()
        {
            var expectedAddressString = $"{UInt160.Zero}";
            var expectedMessageString = $"Wallet account '{expectedAddressString}' is locked.";

            var exception = new NeoBuildWalletAccountLockedException(expectedAddressString);
            var exceptionInterface = exception as INeoBuildException;
            var exceptionBase = exception as Exception;

            // NeoBuildWalletAccountLockedException
            Assert.AreEqual(NeoBuildErrorCodes.Wallet.AccountLocked, exception.ExitCode);
            ...
            // INeoBuildException
            Assert.AreEqual(NeoBuildErrorCodes.Wallet.AccountLocked, exceptionInterface.ExitCode);
            ...
            // System.Exception
            Assert.AreEqual(NeoBuildErrorCodes.Wallet.AccountLocked, exceptionBase.HResult);
