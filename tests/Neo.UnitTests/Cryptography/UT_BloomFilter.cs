...
public class UT_BloomFilter
{
    ...
    public void TestBloomFIlterConstructorGetKMTweak()
    {
        int m = -7, n = 10;
        uint nTweak = 123456;
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new BloomFilter(m, n, nTweak));
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new BloomFilter(m, n, nTweak, new byte[] { 0, 1, 2, 3, 4 }));
    ...
    {
        uint nTweak = 123456;
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new BloomFilter(0, 3, nTweak));
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new BloomFilter(3, 0, nTweak));
    }
}
