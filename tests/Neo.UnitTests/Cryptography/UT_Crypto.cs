...
namespace Neo.UnitTests.Cryptography
{
    ...
    public class UT_Crypto
    {
        ...
        public void TestVerifySignature()
        {
            ...
            Utility.VerifySignature(message, signature, key.PublicKey).Should().BeTrue();

            byte[] wrongKey = new byte[33];
            wrongKey[0] = 0x02;
            Utility.VerifySignature(message, signature, wrongKey, ECCurve.Secp256r1).Should().BeFalse();

            wrongKey[0] = 0x03;
            for (int i = 1; i < 33; i++) wrongKey[i] = byte.MaxValue;
            Action action = () => Utility.VerifySignature(message, signature, wrongKey, ECCurve.Secp256r1);
            action.Should().Throw<ArgumentException>();

            wrongKey = new byte[36];
            action = () => Utility.VerifySignature(message, signature, wrongKey, ECCurve.Secp256r1);
        ...
        public void TestSecp256k1()
        {
            ...
            Utility.VerifySignature(message, signature, pubKey, ECCurve.Secp256k1)
                .Should().BeTrue();
