...
public class UT_IOHelper
{
    ...
    public void TestNullableArray()
    {
        ...
        // Read Error
        Assert.ThrowsExactly<FormatException>(() => MemoryReaderReadNullableArray(data, 2));
        ...
        static void MemoryReaderReadNullableArray(byte[] data, int count)
        {
            var reader = new MemoryReader(data);
            reader.ReadNullableArray<UInt160>(count);
        }
    }
    ...
    public void TestReadVarInt()
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            else
            {
                ...
                var reader = new BinaryReader(stream);
                Assert.ThrowsExactly<FormatException>(() => reader.ReadVarInt(0xFFFFFFFF));
            }
        }
    }
    ...
    public void TestWriteFixedString()
    {
        for (int i = 0; i < 5; i++)
        {
            if (i == 1)
            {
                using var stream = new MemoryStream();
                using var writer = new BinaryWriter(stream);
                Assert.ThrowsExactly<ArgumentException>(() => writer.WriteFixedString("AA", Encoding.UTF8.GetBytes("AA").Length - 1));
            }
            else if (i == 2)
            {
                using var stream = new MemoryStream();
                using var writer = new BinaryWriter(stream);
                Assert.ThrowsExactly<ArgumentException>(() => writer.WriteFixedString("拉拉", Encoding.UTF8.GetBytes("拉拉").Length - 1));
            }
            else if (i == 3)
            {
                using var stream = new MemoryStream();
    ...
    public void TestWriteVarInt()
    {
        for (int i = 0; i < 5; i++)
        {
            if (i == 0)
            {
                using var stream = new MemoryStream();
                using var writer = new BinaryWriter(stream);
                Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => writer.WriteVarInt(-1));
            }
            else if (i == 1)
            {
                using var stream = new MemoryStream();
