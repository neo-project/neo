...
namespace Neo.UnitTests.SmartContract.Native
{
    ...
    public class UT_CryptoLib
    {
        ...
        public void TestVerifyWithECDsa_CustomTxWitness_SingleSig()
        {
            ...
            var tx_signature = Crypto.Sign(tx.GetSignData(TestBlockchain.TheNeoSystem.Settings.Network), privkey, ECCurve.Secp256k1, HashAlgorithmType.Keccak256);
        ...
        public void TestVerifyWithECDsa_CustomTxWitness_MultiSig()
        {
            ...
            for (var i = 0; i < n; i++)
            {
                if (i == 1) // Skip one key since we need only 3 signatures.
                    continue;
                var sig = Crypto.Sign(tx.GetSignData(TestBlockchain.TheNeoSystem.Settings.Network), keys[i].Item1, ECCurve.Secp256k1, HashAlgorithmType.Keccak256);
                inv.EmitPush(sig);
            }
        ...
        public void TestVerifyWithECDsa()
        {
            ...
            // secp256r1 + SHA256
            byte[] signature = Crypto.Sign(message, privR1, ECCurve.Secp256r1, HashAlgorithmType.SHA256);
            ...
            // secp256r1 + Keccak256
            signature = Crypto.Sign(message, privR1, ECCurve.Secp256r1, HashAlgorithmType.Keccak256);
            Crypto.VerifySignature(message, signature, pubR1, HashAlgorithmType.Keccak256).Should().BeTrue();
            CallVerifyWithECDsa(message, pubR1, signature, NamedCurveHash.secp256r1Keccak256).Should().Be(true);

            // secp256k1 + SHA256
            signature = Crypto.Sign(message, privK1, ECCurve.Secp256k1, HashAlgorithmType.SHA256);
            ...
            // secp256k1 + Keccak256
            signature = Crypto.Sign(message, privK1, ECCurve.Secp256k1, HashAlgorithmType.Keccak256);
            Crypto.VerifySignature(message, signature, pubK1, HashAlgorithmType.Keccak256).Should().BeTrue();
