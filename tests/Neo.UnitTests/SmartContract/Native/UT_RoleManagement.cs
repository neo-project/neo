...
public class UT_RoleManagement
{
    ...
    public void TestSetAndGet()
    {
        ...
        foreach (var role in roles)
        {
            ...
            Assert.IsInstanceOfType<Array>(ret, out var array);
            Assert.HasCount(2, array);
            Assert.AreEqual(publicKeys[0].ToArray().ToHexString(), array[0].GetSpan().ToHexString());
            Assert.AreEqual(publicKeys[1].ToArray().ToHexString(), array[1].GetSpan().ToHexString());
            ...
            Assert.IsInstanceOfType<Array>(ret, out array);
