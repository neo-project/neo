...
public partial class UT_InteropService : TestKit
{
    ...
    public void Runtime_GetNotifications_Test()
    {
        ...
        using (var script = new ScriptBuilder())
        {
            ...
            Assert.IsInstanceOfType<Neo.VM.Types.Array>(engine.ResultStack.Peek());

            var array = (Neo.VM.Types.Array)engine.ResultStack.Pop();
    ...
    {
        Assert.IsInstanceOfType<Neo.VM.Types.Array>(stackItem);

        var array = (Neo.VM.Types.Array)stackItem;
    ...
    {
        Assert.IsInstanceOfType<Neo.VM.Types.Array>(GetEngine(true).GetScriptContainer());
    }
    ...
    public void TestRuntime_CheckWitness()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() => engine.CheckWitness(Array.Empty<byte>()));
    }
    ...
    public void TestRuntime_GetCurrentSigners_SysCall()
    {
        ...
        Assert.IsInstanceOfType<Null>(result);
        ...
        result = array[0];
        Assert.IsInstanceOfType<Neo.VM.Types.Array>(result, out array);
        Assert.HasCount(5, array);
        result = array[0]; // Address
    ...
    public void TestContract_Call()
    {
        var snapshotCache = TestBlockchain.GetTestSnapshotCache();
        var method = "method";
        var args = new Neo.VM.Types.Array { 0, 1 };
