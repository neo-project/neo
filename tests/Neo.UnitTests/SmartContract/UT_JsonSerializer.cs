...
public class UT_JsonSerializer
{
    ...
    {
        var json = "[    ]XXXXXXX";
        Assert.ThrowsExactly<FormatException>(() => _ = JToken.Parse(json));
    ...
    public void Deserialize_WrongJson()
    {
        ...
        Assert.ThrowsExactly<FormatException>(() => _ = JsonSerializer.Deserialize(engine, JToken.Parse("x")!, ExecutionEngineLimits.Default));
    }
    ...
    public void Deserialize_EmptyObject()
    {
        var snapshot = _snapshotCache.CloneCache();
        ApplicationEngine engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot);
        var items = JsonSerializer.Deserialize(engine, JToken.Parse("{}")!, ExecutionEngineLimits.Default);

        Assert.IsInstanceOfType<Map>(items);
    ...
    public void Deserialize_EmptyArray()
    {
        ...
        Assert.IsInstanceOfType<Array>(items);
    ...
    public void Deserialize_Map_Test()
    {
        ...
        var items = JsonSerializer.Deserialize(engine, JToken.Parse("{\"test1\":123,\"test2\":321}")!, ExecutionEngineLimits.Default);
    ...
    public void Deserialize_Array_Bool_Str_Num()
    {
        var snapshot = _snapshotCache.CloneCache();
        ApplicationEngine engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, null, ProtocolSettings.Default);
        var items = JsonSerializer.Deserialize(engine, JToken.Parse("[true,\"test\",123,9.05E+28]")!, ExecutionEngineLimits.Default);
    ...
    public void Deserialize_Array_OfArray()
    {
        var snapshot = _snapshotCache.CloneCache();
        ApplicationEngine engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, null, ProtocolSettings.Default);
        var items = JsonSerializer.Deserialize(engine, JToken.Parse("[[true,\"test1\",123],[true,\"test2\",321]]")!, ExecutionEngineLimits.Default);
        ...
        Assert.IsInstanceOfType<Array>(array[0]);
