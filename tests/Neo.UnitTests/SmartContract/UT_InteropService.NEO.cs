...
namespace Neo.UnitTests.SmartContract
{
    public partial class UT_InteropService
    {
        ...
        public void TestContract_Update()
        {
            ...
            nef.CheckSum = NefFile.ComputeChecksum(nef);
            Assert.ThrowsException<InvalidOperationException>(() => snapshot.UpdateContract(null, nef.ToArray(), []));
        ...
        public void TestContract_Update_Invalid()
        {
            ...
            Assert.ThrowsException<InvalidOperationException>(() => snapshot.UpdateContract(null, nefFile.ToArray(),
                [0x01]));
            Assert.ThrowsException<InvalidOperationException>(() => snapshot.UpdateContract(null, nefFile.ToArray(), []));
        }
