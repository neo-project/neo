...
public class UT_Blockchain : TestKit
{
    ...
    public void TestMaliciousOnChainConflict()
    {
        ...
        var key = new KeyBuilder(NativeContract.TokenManagement.Id, 12).Add(accA.ScriptHash).Add(Governance.GasTokenId);
        var entry = snapshot.GetAndChange(key, () => new StorageItem(new AccountState()));
        entry.GetInteroperable<AccountState>().Balance = 100_000_000 * Governance.GasTokenFactor;
        snapshot.Commit();

        key = new KeyBuilder(NativeContract.TokenManagement.Id, 12).Add(accB.ScriptHash).Add(Governance.GasTokenId);
