...
, .. _unit.GetSortedVerifiedTransactions(5)]
            };
        ...
        public async Task UpdatePoolForBlockPersisted_RemoveBlockConflicts()
        {
            ...
            tx6.Signers = [new Signer() { Account = new UInt160(Crypto.Hash160([1, 2, 3])) }, new Signer() { Account = new UInt160(Crypto.Hash160([4, 5, 6])) }];
        ...
        public async Task TryAdd_AddRangeOfConflictingTransactions()
        {
            ...
            long txFee = 1;
            var maliciousSender = new UInt160(Crypto.Hash160([1, 2, 3]));
            var snapshot = GetSnapshot();
            BigInteger balance = NativeContract.GAS.BalanceOf(snapshot, senderAccount);
            ApplicationEngine engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, settings: TestProtocolSettings.Default, gas: long.MaxValue);
            engine.LoadScript(Array.Empty<byte>());
            ...
            malicious.Signers = [new Signer() { Account = new UInt160(Crypto.Hash160([1, 2, 3])), Scopes = WitnessScope.None }];
