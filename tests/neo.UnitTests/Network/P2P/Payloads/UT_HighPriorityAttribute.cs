...
namespace Neo.UnitTests.Network.P2P.Payloads
{
    ...
    public class UT_HighPriorityAttribute
    {
        ...
        public void Verify()
        {
            ...
            Assert.IsFalse(test.Verify(snapshot, new Transaction(ProtocolSettings.Default.Magic) { Signers = new Signer[] { } }));
            Assert.IsFalse(test.Verify(snapshot, new Transaction(ProtocolSettings.Default.Magic) { Signers = new Signer[] { new Signer() { Account = UInt160.Parse("0xa400ff00ff00ff00ff00ff00ff00ff00ff00ff01") } } }));
            Assert.IsTrue(test.Verify(snapshot, new Transaction(ProtocolSettings.Default.Magic) { Signers = new Signer[] { new Signer() { Account = NativeContract.NEO.GetCommitteeAddress(snapshot) } } }));
        }
    }
}
