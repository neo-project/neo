    ...
    public abstract class Wallet
    {
        ...
        public static long CalculateNetWorkFee(byte[] witness_script, ref int size)
        {
            ...
            if (witness_script.IsSignatureContract())
            {
                ...
                networkFee += ApplicationEngine.OpCodePrices[OpCode.PUSHBYTES64] + ApplicationEngine.OpCodePrices[OpCode.PUSHBYTES33] + ApplicationEngine.OpCodePrices[OpCode.PUSHNULL] + InteropService.GetPrice(InteropService.Neo_Crypto_ECDsa_Verify, null);
            }
            else if (witness_script.IsMultiSigContract(out int m, out int n))
            {
                ...
                networkFee += ApplicationEngine.OpCodePrices[OpCode.PUSHNULL] + InteropService.GetPrice(InteropService.Neo_Crypto_ECDsa_Verify, null) * n;
            }
            else
            {
