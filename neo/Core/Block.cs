...
namespace Neo.Core
{
	...
	public class Block : BlockBase, IInventory, IEquatable<Block>
    {
		...
		/// Deserialization
		/// </summary>
		/// <param name="reader">数据来源 Data Source</param>
		...
		/// <summary>
		/// 比较当前区块与指定区块是否相等
		/// Compare the current block with the specified block
		/// </summary>
		/// <param name="other">要比较的区块 The block to be compared</param>
		...
		/// Compare the current block with the specified block
		/// </summary>
		/// <param name="obj">要比较的区块 The block to be compared</param>
		/// <returns>返回对象是否相等 Return whether the object is equal</returns>
		...
		/// <summary>
		/// 获得区块的HashCode
		/// Get the block's HashCode
		/// </summary>
		/// <returns>返回区块的HashCode Block HashCode</returns>
		...
		/// Generate MerkleRoot based on Hash for all transactions in the block
		/// </summary>
		...
		/// Serialization
		/// </summary>
		/// <param name="writer">存放序列化后的数据 Store serialized data</param>
		...
		/// Change the block object to a byte array containing only the block header and the transaction hash to remove the transaction data
		/// </summary>
		/// <returns>返回只包含区块头和交易Hash的字节数组 Return byte array containing only the block header and transaction hash</returns>
		public byte[] Trim()
        {
		...
		/// </summary>
		/// <param name="completely">是否同时验证区块中的每一笔交易 Whether to verify each transaction in the block at the same time</param>
		/// <returns>返回该区块的合法性，返回true即为合法，否则，非法。Returns the legality of the block. Returning true is legal. Otherwise, it is illegal.</returns>
