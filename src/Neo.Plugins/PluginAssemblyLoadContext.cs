...
namespace Neo.Plugins
{
    internal class PluginAssemblyLoadContext : AssemblyLoadContext
    {
        ...
        protected override Assembly? Load(AssemblyName assemblyName)
        {
            ...
            var assemblyFile = Path.Combine(NeoPlugin.PluginDirectory, _pluginName, $"{assemblyName.Name}.dll");
        ...
        {
            var unmanagedDllFilename = GetUnmanagedDllFilename(unmanagedDllName);

            // Checks "Plugins" directory
            var unmanagedDllFile = Path.Combine(NeoPlugin.PluginDirectory, _pluginName, unmanagedDllFilename);
            if (File.Exists(unmanagedDllFile))
            {
                return LoadUnmanagedDllFromPath(unmanagedDllFile);
            }

            // Checks "neo-cli" runtimes directory
            unmanagedDllFile = Path.Combine(
                NeoPlugin.PluginDirectory,
                _pluginName,
                "runtimes",
