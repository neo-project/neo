...
namespace Neo.IO.Actors
{
    internal class PriorityMessageQueue : IMessageQueue, IUnboundedMessageQueueSemantics, IUnboundedDequeBasedMessageQueueSemantics
    {
        ...
        public void Enqueue(IActorRef receiver, Envelope envelope)
        {
            ...
            if (dropper(envelope.Message, preHigh.Concat(high).Concat(preLow).Concat(low).Select(p => p.Message)))
                return;
        ...
        public void EnqueueFirst(Envelope envelope)
        {
            ...
            if (dropper(envelope.Message, preHigh.Concat(high).Concat(preLow).Concat(low).Select(p => p.Message)))
                return;
            ConcurrentStack<Envelope> stack = priority_generator(envelope.Message) ? preHigh : preLow;
            stack.Push(envelope);
        }
    }
}
