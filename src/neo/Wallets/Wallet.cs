    ...
    public abstract class Wallet
    {
        ...
        public static long CalculateNetworkFee(byte[] witness_script, ref int size)
        {
            ...
            if (witness_script.IsSignatureContract())
            {
                ...
                networkFee += ApplicationEngine.OpCodePrices[OpCode.PUSHDATA1] + ApplicationEngine.OpCodePrices[OpCode.PUSHDATA1] + ApplicationEngine.OpCodePrices[OpCode.PUSHNULL] + (InteropService.TryGetPrice(InteropService.Crypto.ECDsaVerify, out long price) ? price : 0);
            }
            else if (witness_script.IsMultiSigContract(out int m, out int n))
            {
                ...
                networkFee += ApplicationEngine.OpCodePrices[OpCode.PUSHNULL] + (InteropService.TryGetPrice(InteropService.Crypto.ECDsaVerify, out long price) ? price : 0) * n;
            }
            else
            {
