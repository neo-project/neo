        ...
        protected internal bool CheckWitnessInternal(UInt160 hash)
        {
            ...
            if (ScriptContainer is Transaction tx)
            {
                ...
                if (signer.Scopes.HasFlag(WitnessScope.CustomCallingGroups))
                {
                    ...
                    var currentGroups = currentContract?.Manifest?.Groups?.Select(p => p.PubKey).ToHashSet() ?? new HashSet<ECPoint>();
                    if (signer.AllowedCallingGroup.Keys.ContainsAny(currentGroups))
                    {
                        if (CallingScriptHash == null || CallingScriptHash == EntryScriptHash) return true;
