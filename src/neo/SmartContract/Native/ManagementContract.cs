...
namespace Neo.SmartContract.Native
{
    public sealed class ManagementContract : NativeContract
    {
        ...
        private ContractState Deploy(ApplicationEngine engine, byte[] nefFile, byte[] manifest)
        {
            ...
            engine.AddGas(ApplicationEngine.StoragePerByteRatio * Policy.GetFeePerByte(engine.Snapshot) * (nefFile.Length + manifest.Length));
        ...
        private void Update(ApplicationEngine engine, byte[] nefFile, byte[] manifest)
        {
            ...
            engine.AddGas(ApplicationEngine.StoragePerByteRatio * Policy.GetFeePerByte(engine.Snapshot) * ((nefFile?.Length ?? 0) + (manifest?.Length ?? 0)));
