    ...
    public sealed class OracleContract : NativeContract
    {
        ...
        internal override void PostPersist(ApplicationEngine engine)
        {
            ...
            foreach (Transaction tx in engine.Snapshot.PersistingBlock.Transactions)
            {
                ...
                nodes ??= RoleDesignation.GetDesignatedByRole(engine.Snapshot, Role.Oracle, engine.Snapshot.PersistingBlock.Index).Select(p => (Contract.CreateSignatureRedeemScript(p).ToScriptHash(), BigInteger.Zero)).ToArray();
                if (nodes.Length > 0)
                {
