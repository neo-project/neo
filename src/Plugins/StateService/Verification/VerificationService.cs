    ...
    public class VerificationService : UntypedActor
    {
        ...
        private void OnTimer(uint index)
        {
            if (contexts.TryGetValue(index, out VerificationContext context))
            {
                ...
                context.Timer = Context.System.Scheduler.ScheduleTellOnceCancelable(TimeSpan.FromMilliseconds((uint)StatePlugin._system.GetBlockGenTime().TotalMilliseconds << context.Retries), Self, new Timer
                {
                    Index = index,
                }, ActorRefs.NoSender);
                context.Retries++;
