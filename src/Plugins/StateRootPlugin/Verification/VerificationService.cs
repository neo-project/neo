...
namespace Neo.Plugins.StateRootPlugin.Verification
{
    public class VerificationService : UntypedActor
    {
        ...
        private void SendVote(VerificationContext context)
        {
            ...
            StateRootPlugin.NeoSystem.Blockchain.Tell(context.VoteMessage);
        }
        ...
        private void CheckVotes(VerificationContext context)
        {
            if (context.IsSender && context.CheckSignatures())
            {
                ...
                StateRootPlugin.NeoSystem.Blockchain.Tell(context.StateRootMessage);
            }
        }
        ...
        private void OnTimer(uint index)
        {
            if (contexts.TryGetValue(index, out VerificationContext context))
            {
                ...
                context.Timer = Context.System.Scheduler.ScheduleTellOnceCancelable(
                    TimeSpan.FromMilliseconds((uint)StateRootPlugin.NeoSystem.GetTimePerBlock().TotalMilliseconds << context.Retries),
                    Self,
