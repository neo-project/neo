...
namespace Neo.Plugins.StateRootPlugin.Storage
{
    ...
    {
        private readonly StateRootPlugin _system;
        ...
        public StateStore(StateRootPlugin system, string path)
        {
            if (_singleton != null) throw new InvalidOperationException(nameof(StateStore));
            _system = system;
            _store = StateRootPlugin.NeoSystem.LoadStore(path);
            _singleton = this;
            StateRootPlugin.NeoSystem.ActorSystem.EventStream.Subscribe(Self, typeof(Blockchain.RelayResult));
        ...
        protected override void OnReceive(object message)
        {
            switch (message)
            {
                ...
                case Blockchain.RelayResult rr:
                    if (rr.Result == VerifyResult.Succeed && rr.Inventory is ExtensiblePayload payload && payload.Category == StateRootPlugin.StatePayloadCategory)
        ...
        private bool OnNewStateRoot(StateRoot stateRoot)
        {
            ...
            if (!stateRoot.Verify(StateRootPlugin.NeoSystem.Settings, StateRootPlugin.NeoSystem.StoreView)) return false;
        ...
        public static Props Props(StateRootPlugin system, string path)
        {
