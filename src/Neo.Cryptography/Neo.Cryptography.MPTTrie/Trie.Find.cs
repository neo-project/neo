                ...
                case NodeType.BranchNode:
                    {
                        ...
                        return Utility.Utility.Concat(path[..1], Seek(ref node.Children[path[0]], path[1..], out start));
                    }
                case NodeType.ExtensionNode:
                    {
                        ...
                        {
                            return Utility.Utility.Concat(node.Key.Span, Seek(ref node.Next, path[node.Key.Length..], out start));
                        }
                ...
                case NodeType.BranchNode:
                    {
                        if (offset < from.Length)
                        {
                            ...
                            {
                                if (from[offset] < i)
                                    foreach (var item in Travers(node.Children[i], Utility.Utility.Concat(path, new byte[] { (byte)i }), from, from.Length))
                                        yield return item;
                                else if (i == from[offset])
                                    foreach (var item in Travers(node.Children[i], Utility.Utility.Concat(path, new byte[] { (byte)i }), from, offset + 1))
                                        yield return item;
                            }
                        }
                        else
                        {
                            ...
                            for (int i = 0; i < Node.BranchChildCount - 1; i++)
                            {
                                foreach (var item in Travers(node.Children[i], Utility.Utility.Concat(path, new byte[] { (byte)i }), from, offset))
                                    yield return item;
                            }
                        }
                        break;
                ...
                case NodeType.ExtensionNode:
                    {
                        if (offset < from.Length && from.AsSpan()[offset..].StartsWith(node.Key.Span))
                            foreach (var item in Travers(node.Next, Utility.Utility.Concat(path, node.Key.Span), from, offset + node.Key.Length))
                                yield return item;
                        else if (from.Length <= offset || 0 < node.Key.Span.CompareTo(from.AsSpan(offset)))
                            foreach (var item in Travers(node.Next, Utility.Utility.Concat(path, node.Key.Span), from, from.Length))
                                yield return item;
                        break;
                    }
            }
