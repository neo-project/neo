...
namespace Neo.Persistence
{
    ...
    public class StoreCache : DataCache, IDisposable
    {
        ...
        protected override IEnumerable<(StorageKey, StorageItem)> SeekInternal(byte[] keyOrPrefix, SeekDirection direction, int start)
        {
            return _store.Find(keyOrPrefix, direction, start).Select(p => (new StorageKey(p.Key), new StorageItem(p.Value)));
        }
