...
namespace Neo.Json
{
    sealed class JPathToken
    {
        ...
        public static void ProcessJsonPath(ref JsonNode?[] objects, Queue<JPathToken> tokens)
        {
            var maxDepth = 6;
            var maxObjects = 1024;
        ...
        private static void ProcessDot(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, Queue<JPathToken> tokens)
        {
            var token = DequeueToken(tokens);
            switch (token.Type)
            {
        ...
        private static void ProcessBracket(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, Queue<JPathToken> tokens)
        {
            var token = DequeueToken(tokens);
            switch (token.Type)
            {
                ...
                case JPathTokenType.String:
                    ...
                    switch (next.Type)
                    {
                        case JPathTokenType.Comma:
                            ...
                            break;
                        case JPathTokenType.RightBracket:
                            Descent(ref objects, ref maxDepth, maxObjects, JsonNode.Parse($"\"{token.Content!.Trim('\'')}\"")!.GetValue<string>());
                            break;
                        default:
                            throw new FormatException($"Unexpected token {next.Type}");
                    }
        ...
        private static void ProcessRecursiveDescent(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, Queue<JPathToken> tokens)
        {
            ...
            {
                results.AddRange(objects.OfType<JsonObject>().SelectMany(p => p).Where(p => p.Key == token.Content).Select(p => p.Value));
                Descent(ref objects, ref maxDepth, maxObjects);
        ...
        private static void ProcessSlice(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, Queue<JPathToken> tokens, int start)
        {
            var token = DequeueToken(tokens);
            switch (token.Type)
            {
        ...
        private static void ProcessUnion(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, Queue<JPathToken> tokens, JPathToken first)
        {
            ...
            switch (first.Type)
            {
                case JPathTokenType.Number:
                    ...
                    break;
                case JPathTokenType.String:
                    Descent(ref objects, ref maxDepth, maxObjects, items.Select(p => JsonNode.Parse($"\"{p.Content!.Trim('\'')}\"")!.GetValue<string>()).ToArray());
                    break;
                default:
        ...
        private static void Descent(ref JsonNode?[] objects, ref int maxDepth, int maxObjects)
        {
            if (maxDepth <= 0)
                throw new InvalidOperationException("Exceeded max depth");
            --maxDepth;

            objects = objects.SelectMany(p => p switch
            {
                IDictionary<string, JsonNode?> o => o.Values,
                JsonArray a => a,
        ...
        private static void Descent(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, params string[] names)
        {
            static IEnumerable<JsonNode?> GetProperties(JsonObject obj, string[] names)
            {
            ...
            objects = objects.OfType<JsonObject>().SelectMany(p => GetProperties(p, names)).ToArray();
            if (objects.Length > maxObjects)
                throw new InvalidOperationException(nameof(maxObjects));
        }

        private static void Descent(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, params int[] indexes)
        {
            static IEnumerable<JsonNode?> GetElements(JsonArray array, int[] indexes)
            {
            ...
            objects = objects.OfType<JsonArray>().SelectMany(p => GetElements(p, indexes)).ToArray();
            if (objects.Length > maxObjects)
                throw new InvalidOperationException(nameof(maxObjects));
        }

        private static void DescentRange(ref JsonNode?[] objects, ref int maxDepth, int maxObjects, int start, int end)
        {
            if (maxDepth <= 0)
                throw new InvalidOperationException("Exceeded max depth");
            --maxDepth;

            objects = objects.OfType<JsonArray>().SelectMany(p =>
            {
