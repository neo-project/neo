...
public sealed class Governance : NativeContract
{
    ...
    internal override async ContractTask InitializeAsync(ApplicationEngine engine, Hardfork? hardFork)
    {
        if (hardFork == ActiveIn)
        {
            ...
            await TokenManagement.MintInternal(engine, tokenid, account, engine.ProtocolSettings.InitialGasDistribution, assertOwner: false, callOnBalanceChanged: false, callOnPayment: false, callOnTransfer: false);
        }
    }
