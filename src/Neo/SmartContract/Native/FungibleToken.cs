...
namespace Neo.SmartContract.Native
{
        ...
        where TState : AccountState, new()
    {
        ...
        internal async ContractTask Mint(ApplicationEngine engine, UInt160 account, BigInteger amount, bool callOnPayment)
        {
            ...
            StorageItem storage = engine.Snapshot.GetAndChange(CreateStorageKey(Prefix_Account).Add(account), () => new StorageItem(new TState())).NullExceptionOr<StorageItem>();
            TState state = storage.GetInteroperable<TState>();
            OnBalanceChanging(engine, account, state, amount);
            state.Balance += amount;
            storage = engine.Snapshot.GetAndChange(CreateStorageKey(Prefix_TotalSupply), () => new StorageItem(BigInteger.Zero)).NullExceptionOr<StorageItem>();
            storage.Add(amount);
        ...
        internal async ContractTask Burn(ApplicationEngine engine, UInt160 account, BigInteger amount)
        {
            ...
            StorageItem storage = engine.Snapshot.GetAndChange(key).NullExceptionOr<StorageItem>();
            TState state = storage.GetInteroperable<TState>();
            ...
            if (state.Balance == amount)
                ...
                state.Balance -= amount;
            storage = engine.Snapshot.GetAndChange(CreateStorageKey(Prefix_TotalSupply)).NullExceptionOr<StorageItem>();
            storage.Add(-amount);
        ...
        private protected async ContractTask<bool> Transfer(ApplicationEngine engine, UInt160 from, UInt160 to, BigInteger amount, StackItem data)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    StorageItem storage_to = engine.Snapshot.GetAndChange(key_to, () => new StorageItem(new TState())).NullExceptionOr<StorageItem>();
