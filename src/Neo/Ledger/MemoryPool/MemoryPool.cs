...
public partial class MemoryPool : IReadOnlyCollection<Transaction>
{
    ...
    public int Count
    {
        get
        {
            ...
            try
            {
                return _unsortedVerifiedTransactions.Count + _unverifiedTransactions.Count;
            }
            finally
            {
    ...
    public bool ContainsKey(UInt256 hash)
    {
        ...
        try
        {
            return _unsortedVerifiedTransactions.ContainsKey(hash) || _unverifiedTransactions.ContainsKey(hash);
        }
        finally
        {
            _txRwLock.ExitReadLock();
    ...
    public bool TryGetValue(UInt256 hash, out Transaction tx)
    {
        ...
        try
        {
            bool ret = _unsortedVerifiedTransactions.TryGetValue(hash, out PoolItem item)
    ...
    public IEnumerator<Transaction> GetEnumerator()
    {
        ...
        try
        {
            return _unsortedVerifiedTransactions.Select(p => p.Value.Tx)
        ...
        out IEnumerable<Transaction> unverifiedTransactions)
    {
        ...
        try
        {
            verifiedTransactions = _sortedVerifiedTransactions.Reverse().Select(p => p.Tx).ToArray();
    ...
    private PoolItem GetLowestFeeTransaction(out Dictionary<UInt256, PoolItem> unsortedTxPool, out SortedSet<PoolItem> sortedPool)
    {
        ...
        try
        {
            return GetLowestFeeTransaction(_sortedVerifiedTransactions, _unverifiedSortedTransactions, out sortedPool);
        }
        finally
        {
            unsortedTxPool = Object.ReferenceEquals(sortedPool, _unverifiedSortedTransactions)
                ? _unverifiedTransactions : _unsortedVerifiedTransactions;
        }
    }
    ...
    internal VerifyResult TryAdd(Transaction tx, DataCache snapshot)
    {
        ...
        if (_unsortedVerifiedTransactions.ContainsKey(tx.Hash)) return VerifyResult.AlreadyExists;
        ...
        try
        {
            ...
            _unsortedVerifiedTransactions.Add(tx.Hash, poolItem);
            VerificationContext.AddTransaction(tx);
            _sortedVerifiedTransactions.Add(poolItem);
        ...
        if (!_unsortedVerifiedTransactions.ContainsKey(tx.Hash)) return VerifyResult.OutOfMemory;
    ...
    private List<Transaction> RemoveOverCapacity()
    {
        ...
        do
        {
            ...
            if (ReferenceEquals(sortedPool, _sortedVerifiedTransactions))
    ...
    internal void UpdatePoolForBlockPersisted(Block block, DataCache snapshot)
    {
        ...
        ReverifyTransactions(_sortedVerifiedTransactions, _unverifiedSortedTransactions, (int)_system.Settings.MaxTransactionsPerBlock, MaxMillisecondsToReverifyTx, snapshot);
    }
        ...
        SortedSet<PoolItem> unverifiedSortedTxPool, int count, double millisecondsTimeout, DataCache snapshot)
    {
        ...
        try
        {
            ...
            {
                if (_unsortedVerifiedTransactions.TryAdd(item.Tx.Hash, item))
                {
    ...
    internal bool ReVerifyTopUnverifiedTransactionsIfNeeded(int maxToVerify, DataCache snapshot)
    {
        ...
        {
            int verifyCount = _sortedVerifiedTransactions.Count > _system.Settings.MaxTransactionsPerBlock ? 1 : maxToVerify;
            ReverifyTransactions(_sortedVerifiedTransactions, _unverifiedSortedTransactions,
