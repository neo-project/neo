...
//
// SyscallSuite.cs file belongs to the neo project and is free
...
{
    public class SyscallSuite : VmBenchmarkSuite
    {
        ...
        public void SuiteSetup()
        {
            ...
            _artifactPath = Path.Combine(root, $"syscall-metrics-{DateTime.UtcNow:yyyyMMddHHmmss}.csv");
        }
        ...
        public void SuiteCleanup()
        {
            ...
            summary.Write();
            BenchmarkArtifactRegistry.RegisterMetrics(BenchmarkComponent.Syscall, _artifactPath);

            var coverageRoot = Environment.GetEnvironmentVariable("NEO_BENCHMARK_ARTIFACTS")
                               ?? Path.Combine(AppContext.BaseDirectory, "BenchmarkArtifacts");
            var coveragePath = Path.Combine(coverageRoot, "syscall-missing.csv");
            var missing = SyscallCoverageReport.GetMissing();
            InteropCoverageReport.WriteReport(coveragePath, missing, System.Array.Empty<string>());
            BenchmarkArtifactRegistry.RegisterCoverage("syscall-missing", coveragePath);

        }

        protected override IEnumerable<VmBenchmarkCase> GetCases()
        {
            return SyscallScenarioFactory.CreateCases();
        }
    }
}
