...
## Developer Guidance
We try to have as few rules as possible, only 5, just enough to keep the project organized:


1.  **Discuss before coding**
Proposals must be discussed before being implemented.  
Avoid implementing issues with the discussion tag.
2. **Tests during code review**
We expect reviewers to test the issue before approving or requesting changes.

3. **Give time to other developers review an issue**
Even if the code has been approved, you should leave at least 24 hours for others to review it before merging the code.

4. **Create unit tests**
It is important that the developer includes basic unit tests so reviewers can test it.
